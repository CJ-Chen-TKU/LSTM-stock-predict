...
def load_data_with_indexes(symbol, start, end):
    ...
    # 確保 QQQ_Close 與 VIX_Close 為 Series
    qqq.columns = ["QQQ_Close"]
    vix.columns = ["VIX_Close"]
    if isinstance(qqq["QQQ_Close"], pd.DataFrame):
        qqq["QQQ_Close"] = qqq["QQQ_Close"].iloc[:, 0]
    if isinstance(vix["VIX_Close"], pd.DataFrame):
        vix["VIX_Close"] = vix["VIX_Close"].iloc[:, 0]
