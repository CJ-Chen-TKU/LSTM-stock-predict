...
def load_data_with_indexes(symbol, start, end):
    ...
    st.write(f"Downloaded {symbol} stock data shape: {stock.shape}")
   
    st.write(stock.columns) 
    
    if stock.empty:
        st.warning(f"No stock data for {symbol} between {start} and {end}")
        return pd.DataFrame()
    ...
    if "Close" not in stock.columns:
        st.warning(f"No 'Close' column for {symbol}")
        return pd.DataFrame()
    ...
    st.write(f"Downloaded QQQ shape: {qqq.shape}, VIX shape: {vix.shape}")

    df = stock.join([qqq, vix], how="outer")
    st.write(f"Joined DataFrame shape: {df.shape}")
...
if run_train:
    ...
    end_for_download = max(end_date, forecast_target_date + pd.Timedelta(days=1))
