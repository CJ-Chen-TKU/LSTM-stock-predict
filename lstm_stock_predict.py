...
if st.button("🚀 Train and Predict"):
    ...
    if len(seq_data) == sequence_length:
        ...
        # --- Plot with Buy marker ---
        plot_data = data.loc[data.index >= forecast_target_date - pd.Timedelta(days=sequence_length + 5)].copy()
        plot_data = plot_data.loc[plot_data.index <= forecast_target_date + pd.Timedelta(days=3)]
        plot_data["Buy_Signal"] = np.nan
        if next_day_pred > 0.5 and forecast_target_date in plot_data.index:
            plot_data.loc[forecast_target_date, "Buy_Signal"] = plot_data.loc[forecast_target_date, "Close"]

        st.subheader("📍 Entry Signal Visualization")
        st.line_chart(plot_data[["Close", "Buy_Signal"]])
    else:
        st.write(f"Insufficient data for forecasting {forecast_target_date.date()} (need {sequence_length} days).")
