...
if st.session_state.loaded_data is not None:
    if st.button("🚀 Train and Predict"):
        ...
        last_seq = torch.tensor(test_data[features].values[-sequence_length:], dtype=torch.float32).unsqueeze(0).to(device)
        next_day_pred = model(last_seq).item()
        st.write(f"Probability of going up tomorrow: {next_day_pred:.2%}")
