...
if st.session_state.loaded_data is not None:
    if st.button("🚀 Train and Predict"):
        ...
        seq_data = data.loc[:forecast_day - pd.Timedelta(days=1)].tail(sequence_length)[features]
        ...
        if len(seq_data) == sequence_length:
            ...
            st.write(f"Insufficient trading data for forecasting next day (need {sequence_length} days, got {len(seq_data)}).")
