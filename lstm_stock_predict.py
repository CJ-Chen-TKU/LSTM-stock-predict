...
def load_data_with_indexes(symbol, start, end):
    stock = yf.download(symbol, start=start, end=end, progress=False, group_by="ticker")
    if isinstance(stock.columns, pd.MultiIndex):
        stock.columns = stock.columns.droplevel(0)

    qqq = yf.download("QQQ", start=start, end=end, progress=False, group_by="ticker")
    if isinstance(qqq.columns, pd.MultiIndex):
        qqq.columns = qqq.columns.droplevel(0)
    qqq = qqq[["Close"]].rename(columns={"Close": "QQQ_Close"})

    vix = yf.download("^VIX", start=start, end=end, progress=False, group_by="ticker")
    ...
    close = df["Close"]
    tomorrow_close = df["Tomorrow_Close"]
    tomorrow_close_aligned, close_aligned = tomorrow_close.align(close, join="inner")
    df.loc[tomorrow_close_aligned.index, "Target"] = (tomorrow_close_aligned > close_aligned).astype(int)
...
if st.button("📈 Show Correlation Matrix"):
    data = load_data_with_indexes(ticker, start_date, end_date)
    if data.empty:
        st.error("⚠️ No data to correlate.")
    else:
        corr = data[["Close", "SMA_10", "SMA_50", "RSI", "MACD", "MACD_signal", "QQQ_pct", "VIX_pct"]].corr()
        fig, ax = plt.subplots()
...
if st.session_state.loaded_data is not None:
    if st.button("🚀 Train and Predict"):
        ...
        train_data = data[(data.index >= pd.to_datetime(train_start_date)) & (data.index <= pd.to_datetime(train_end_date))]
        test_data = data[data.index > pd.to_datetime(train_end_date)]

        if len(train_data) < sequence_length + 1 or len(test_data) < sequence_length + 1:
        ...
        for epoch in range(epochs):
            ...
            epoch_text.text(f"Epoch {epoch+1}/{epochs}, Train Loss: {loss:.4f}, Test Acc: {acc:.4f}")
        ...
        test_data_plot = test_data.iloc[sequence_length:].copy()
        test_data_plot["Prediction"] = preds
        test_data_plot["Target"] = targets

        st.subheader("📉 Close Price")
        st.line_chart(test_data_plot["Close"])

        st.subheader("🧠 Predicted vs Actual Direction")
        st.line_chart(test_data_plot[["Target", "Prediction"]])

        st.subheader("🔮 Forecast for Next Day")
        # Prediction input sequence based on train_end_date and sequence_length only:
        forecast_day = pd.to_datetime(train_end_date) + pd.Timedelta(days=1)
        forecast_seq_start = forecast_day - pd.Timedelta(days=sequence_length)

        seq_data = data.loc[forecast_seq_start:forecast_day - pd.Timedelta(days=1), features]

        if len(seq_data) == sequence_length:
            last_seq = torch.tensor(seq_data.values, dtype=torch.float32).unsqueeze(0).to(device)
            next_day_pred = model(last_seq).item()
            st.write(f"Probability of going up on {forecast_day.date()}: {next_day_pred:.2%}")
        else:
            st.write(f"Insufficient data for forecasting next day from {forecast_seq_start.date()} to {(forecast_day - pd.Timedelta(days=1)).date()}")
