...
if st.session_state.loaded_data is not None:
    if st.button("🚀 Train and Predict"):
        ...
        # New forecast target date input
        ...
        # Use sequence_length trading days immediately before forecast_target_date
        seq_data = data.loc[:forecast_target_date - pd.Timedelta(days=1)].tail(sequence_length)[features]
        ...
        if len(seq_data) == sequence_length:
            ...
            st.write(f"Probability of going up on {forecast_target_date.date()}: {next_day_pred:.2%}")
        else:
            st.write(f"Insufficient trading data for forecasting {forecast_target_date.date()} (need {sequence_length} days, got {len(seq_data)}).")
